...
# 1 point for using relative paths
```
...
#1 point for the right answer, mmr per a country per a year should be good too
...
```{r}
head(mmr_data, n = 10)
# 1 point for justifcation of answer
```
...
# 1 pt for "names_to = "years""
# 1 pt for "Valuess to = mmr"
# 1 pt for dropping na values
```
...
```{r}
anyNA(mmr_tidy)
# 1 pt for showing there are no NA values
```
...
```{r}
nrow(mmr_tidy)
# 1 point for 164, 0 for anything else
```
...
```{r}
class(mmr_tidy$year)
# 1 pt for acknowledging that year being a character is not normal
```
...
# 1 pt for successfully using mutate and saving their numeric year
```
...
11) What is the difference between a `full_join` and a `left_join`? Answer this question in the context of explaining the difference between the outputs for question 9) and 10).  (2 pts)
 ...
 # 1 pt for acknowleding difference in datasets ( 1 has 4 less rows than the other). 
 # 1 pt for explaining difference between left join and full join
...
# 1 pt for the graph being faceted by continent
# 1 pt for t being interactive
```
...
left_table_tidy %>%
  ...
  summarize(n_countries = length(unique(country)),
            ...
            max_mmr = max(mmr),
            min_mmr = min(mmr))
# 1 pt for grouping by continent
# 1 pt for showing max, min, total number of couneries, total number of observations (remove 0.25 for each they are missing)
```
...
# 1 pt for having multiple columns per a year (1990, 1991, 1992, etc)
# 1 pt for having the observations be by country
```
