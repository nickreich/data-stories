...
date: "Due: March 3rd 6:00 pm, 2023"
output:
...
* Download the data called ``gapminder.csv`` from Moodle (Unit 2 > Unit 2 readings and files folder).
...
```{r, fig.show='hide', include=FALSE, eval=FALSE}
theme_set(theme_bw())
p
```
...
```{r, fig.show='hide', eval=FALSE, include=FALSE}
ggplotly()
```
