...
date: "Due: 6pm, Friday February 10th, 2023"
output:
...
* Create new subdirectories in the `PH460` directory you just created. They should be called `coding-challenges` and `data`.
...
* Download the `women.csv` from the Moodle assignment and save it in the `data` folder you created above in the project.
* If you are new to RMarkdown, we recommend reviewing [this overview on using RMarkdown files in RStudio](https://rmarkdown.rstudio.com/authoring_quick_tour.html).


__Answer the questions below for credit__

By the deadline listed above, you must hand in both the .Rmd file and the resulting, "knitted" HTML file on Moodle. The HTML file must show all code and output  (2 pts). Hint: this involves making sure the `echo` option is set to `TRUE` in your RMarkdown file, which it should be by default.
