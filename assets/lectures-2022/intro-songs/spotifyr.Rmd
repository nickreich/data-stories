...
You should create a new `.R` script file (note, DO NOT create a `.Rmd` file). Make a new directory for "in-class code" in your root directory for this project. You could call it `class-code` or something like that. Save this R file in that directory. Copy the relevant and necessary code from below into your script as you go, editing as needed.
...
2. Register your account with [the Spotify for Developers system](https://developer.spotify.com/dashboard/login).
3. Once you've linked your account as a Spotify "developer" then you should be able to visit [this Dashboard page](https://developer.spotify.com/dashboard/applications) and see a green button to "Create an app". Click that green button.
...
The following packages will be used in this tutorial. Note that the following lines will install packages and you should run them once in your console and should NOT include them in your final Rmd file, as then the packages would be installed every time the file is knit. You only need to install them once and LOAD them via `library()` every time the file is knit.
