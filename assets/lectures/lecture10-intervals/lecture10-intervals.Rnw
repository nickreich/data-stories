...
Assuming $H_0:\beta_{age} = 0$ is true we can use the estimated SE to approximate the sampling distribution as a t-distribution
...
Using a sample of 100 observations from the population:
\scriptsize
<<message=FALSE>>=
library(mosaic)
