...
\input{../lecture5-slr/lec_style.tex}
%%%%%%%% END FROM TEX FILE
...
\begin{frame}
\frametitle{What is a reasonable model}

\begin{block}{In generic model syntax}
\end{block}

\vspace{6em}

\begin{block}{In regression formula syntax}
\end{block}
...
\soln{ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ , kids with mothers whose IQs are one point higher tend to score on average 0.56 points higher.}
...
\soln{\only<2>{Kids whose moms haven't gone to HS, whose moms have an IQ of 0, and who are 0 yrs old are expected on average to score \Sexpr{round(coef(fm)[1],2)}. Obviously, the intercept does not make any sense in context.}}
...
\pq{What is the correct interpretation of the slope for \var{mom\_hs}?}
...
All else being equal, kids whose moms graduated from high school are estimated to score 
\vspace{2em}
...
\scriptsize
Note: the actual dataset has slightly different variable names than those in the slides. More details on the dataset can be found here \href{https://www.openintro.org/stat/data/?data=cc}{https://www.openintro.org/stat/data/?data=cc}.
