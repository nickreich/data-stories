...
\begin{frame}[fragile]{Reading model values}
...
Broadly speaking, is there a relationship between crowding and disease?
...
\begin{frame}[fragile]{Formulas for Statistical Models}
...
<<main-term-smoking, tidy=FALSE, fig.height=4, echo=FALSE>>=
ggplot(dat, aes(factor(smoking), disease)) + #geom_point() + 
    geom_boxplot()
@
...
$$ {\mbox model: }  disease \sim 1 + crowding * smoking $$
...
Linear models are not the only kind of models!
